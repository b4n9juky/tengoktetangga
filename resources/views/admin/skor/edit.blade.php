    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                ...
                <form method="post" action="{{route('skor.update',$row->id) }}">
                    @csrf

                    <div class="mb-4">
                        <label class="block font-medium text-sm text-gray-700">Skor Awal</label>
                        <input type="number" name="nilai_awal" value="{{$row->nilai_awal}}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div class="mb-4">
                        <label class="block font-medium text-sm text-gray-700">Skor Akhir</label>
                        <input type="number" name="nilai_akhir" value="{{$row->nilai_akhir}}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div class="mb-4">
                        <label class="block font-medium text-sm text-gray-700">Keterangan</label>
                        <input type="text" name="keterangan" value="{{$row->keterangan}}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
